...
const CRUMBS: Record<string, string> = {
  ...
  '/button': 'Button',
  '/chip': 'Chip',
  '/clickable-option': 'Clickable Option',
...
function DocHeader() {
  ...
  const crumb = CRUMBS[pathname] ?? 'Not found'
  const { preview, setPreview } = useContext(PreviewContext)

  return (
    <header className="flex shrink-0 items-center justify-between gap-4 bg-black px-6 py-3 text-white">
